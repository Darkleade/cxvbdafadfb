...
async function saveDatas(user, channel, data) {
  ...
  if (channel.parent && conf.publicParents.includes(channel.parentID)) {
    if (data >= (1000 * 60) * conf.voiceCount) await coin.findOneAndUpdate({ guildID: user.guild.id, userID: user.id }, { $inc: { coin: 0.50 * parseInt(data/1000/60) } }, { upsert: true });
  } else if (data >= (1000 * 60) * conf.voiceCount) await coin.findOneAndUpdate({ guildID: user.guild.id, userID: user.id }, { $inc: { coin: 0.15 * parseInt(data/1000/60) } }, { upsert: true });
